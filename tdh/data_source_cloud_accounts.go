...
func (d *cloudAccountsDatasource) Schema(_ context.Context, _ datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	resp.Schema = schema.Schema{
		Description: "Used to fetch all cloud accounts on TDH for BYOC.",
...
func (d *cloudAccountsDatasource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	query := &infra_connector.CloudAccountsQuery{}

	cloudAccounts, err := d.client.InfraConnector.GetCloudAccounts(query)
	if err != nil {
		resp.Diagnostics.AddError(
			"Unable to Read TDH Cloud Accounts",
			err.Error(),
		)
		return
	}
...
func (d *cloudAccountsDatasource) convertToTfModel(ctx context.Context, cloudAccountDto model.CloudAccount, resp *datasource.ReadResponse) (cloudAccountModel, bool) {
